...
func (r *TrainingRepository) GetTrainingByID(
	...
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(id, "training"), "get training by id")
	}
...
func (r *TrainingRepository) StartTraining(
	...
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(userID, "user"), "start training")
	}
	td := trainingData{
		UserID:    ouID,
		StartTime: startTime,
	...
	results, err := r.col.InsertOne(ctx, td)
	if err != nil {
...
func (r *TrainingRepository) EndTraining(
	...
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(id, "training"), "end training")
	}

	opts := options.FindOneAndUpdate().SetReturnDocument(options.After)
	update := bson.M{"$set": bson.M{"end_time": endTime}}

	results := r.col.FindOneAndUpdate(
		ctx, trainingData{ID: tOID}, update, opts)
	err = results.Err()
	if err != nil {
...
func (r *TrainingRepository) GetUserTrainings(
	...
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(userID, "user"),
			"get user trainings")
	}
...
func (r TrainingRepository) StartExercise(
	ctx context.Context,
	trID string,
	...
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(trID, "training"),
			"start exercise")
	}

	exOID, err := primitive.ObjectIDFromHex(exercise.ExerciseID)
	if err != nil {
		return nil, usecases.NewErrorInvalidID(exercise.ExerciseID, "exercise")
	}
	...
	results, err := r.col.UpdateOne(ctx, filter, update)
	if err != nil {
...
func (r TrainingRepository) AddSet(
	...
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(teID, "training exercise"), "add set")
	}
	uOID, err := primitive.ObjectIDFromHex(userID)
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(userID, "user"), "add set")
	}
...
func (r TrainingRepository) GetTrainingExercises(
	...
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(id, "training"),
			"get training exercises")
	}

	filter := bson.M{"_id": tOID}

	c, err := r.col.Find(ctx, filter)
	if err != nil {
...
func (r TrainingRepository) EndExercise(
	...
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(id, "training"), "end exercises")
	}
	uOID, err := primitive.ObjectIDFromHex(userID)
	if err != nil {
		return nil, errors.WithMessage(
			usecases.NewErrorInvalidID(id, "user"), "end exercises")
	}
	...
	result := r.col.FindOneAndUpdate(ctx, filter, update, opts)
	if err = result.Err(); err != nil {
